...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.user.role !== 'MENTOR') {
      return res.status(403).json({ error: 'Only mentors can accept connection requests' });
    }
    ...
    if (connection.mentorId !== parseInt(req.user.id, 10)) {
      return res.status(403).json({ error: 'You are not authorized to accept this connection' });
    }
